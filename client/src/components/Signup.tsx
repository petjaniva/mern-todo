    ...
    return (
        <div>
            ...
            <div className="flex justify-between items-centered">
                <div>
                    <p>Already a member?<span onClick={renderLogin} className="text-green-400 cursor-pointer">Login</span></p>
                </div>
                <button className={`rounded-lg px-6 py-2 font-bold text-white ${disabled ? "bg-gray-400" : "bg-green-400"}`} disabled={disabled} onClick={() => onSubmit()}>Signup</button>
            </div>
        </div>
        </div>
    )
