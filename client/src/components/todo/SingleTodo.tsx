...
const SingleTodo: React.FC<ITodoProps> = (props: ITodoProps) => {
  ...
  const getUserEmail = (userId: Types.ObjectId): string => {
    ...
    axios.get(`/user/${userId}`, { headers: { token: token } }).then((res) => {
      user = res.data.user;
    }).catch((err) => {
      console.log(err);
    }
...
// }
  return (
    ...
    >
      ...
      <input
        type="button"
        className={(props.todo.isWorkedOn ? "text-blue-400" : "text-white") + "py-2 px-4 bg-green-400  rounded-md cursor-pointer"}
